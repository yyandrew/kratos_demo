...
func callHTTP() {
	conn, err := transhttp.NewClient(
		context.Background(),
		transhttp.WithMiddleware(recovery.Recovery()),
	...
	reply, err := client.SayHello(context.Background(), &pb.HelloRequest{Name: "andrew"})
	...
	if errors.IsBadRequest(err) {
		log.Printf("[http] SayHello error is invalid argumen: %v\n", err)
...
func callGRPC() {
	conn, err := transgrpc.DialInsecure(
		...
		transgrpc.WithMiddleware(recovery.Recovery()),
	)
	...
	if errors.IsBadRequest(err) {
		log.Printf("[grpc] SayHello error is invalid argumen: %v\n", err)
	}
}
